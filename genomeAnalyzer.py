...
# Executable: python genomeAnalyzer.py < testGenome.fa
...
#####################################################################################################
# Main(): contains genomeAnalyser
#####################################################################################################
'''
Genome analyzer is included and developed throughout the main() function.
'''
def main():
    '''
    Funtion reads over the input fasta file and executes fucntions. Function calculates the 
    sequence length, GC content, and amino acids compositions.
    '''
    aaCodonTable = {} # creates an amino acids dictionary
    myReader =  FastAreader()  #'testGenome.fa') # make sure to change this to use stdin
    nucParam = NucParams()
    # iterates over header and sequence in input file
    for header, sequence in myReader.readFasta():
        nucParam.addSequence(sequence)
        codonComp = nucParam.nucComposition()
        dnaCount = nucParam.nucCount()
        # calculates the number of Gs and Cs in sequence
        gcContent = codonComp['G'] + codonComp['C']
        # calculate GC content 
        gcContent = (gcContent/dnaCount)*100
        pass

    # calculates the sequence length and returns it in Mb
    print("Sequence length = {:0.2f} Mb\n".format(dnaCount*(1/10**6))) # convert to Mb by multiplying (1/1000000)
    # print the percentage of G and C characters in testGenome.fa
    print("GC content = {:0.2f}%\n".format(gcContent))
    pass
   
    # creates an codons : amino acids dictionary
    for codon, aa in nucParam.rnaCodonTable.items():
        # if amino acid in dictionary keys, keys equal to codon
        if aa in aaCodonTable.keys():
            aaCodonTable[aa].append(codon) # adds codons in dictionary
        else:
            aaCodonTable[aa] = [codon] # saves anyother amino acid
        pass

    # acccess codonComposition() function in nucParams class  
    codonCount = nucParam.codonComposition()
    # iterate over codon counts and amino acids in dictionary 
    for codons in sorted(codonCount):
        # access and sorts items in AA dictionary
        char = nucParam.rnaCodonTable[codons] 
        # calculates relative codon usage for each codon
        composition = codonCount[codons]
        # iterates over amino acids
        for letter in char:
            aminoCount = nucParam.aaComp[letter]
            if composition != 0: # if composition is not zero, calculates the codon frequency 
                frequency = composition/ aminoCount # calculates the amino acids frequency
            else:
                frequency = composition/1 # calculates the amino acid frequency when composition is zero
        
        print('{} : {} {:5.1f} ({:6d})'.format(codons, char, frequency*100, composition))
